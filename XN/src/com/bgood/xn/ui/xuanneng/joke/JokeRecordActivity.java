...
public class JokeRecordActivity extends BaseShowDataActivity implements OnClickListener, OnItemClickListener,TaskListenerWithState,IXListViewListener
{
	...
	private void requestData(int dataType,int start,int end) {
		XuannengRequest.getInstance().requestXuanRecord(this, mActivity, XuanNengMainActivity.XUANNENG_JOKE, dataType, start, end,BGApp.location.longitude, BGApp.location.latitude);
	}
     ...
     * @author:hg_liuzl@163.com
     * @params:@param timevalue
     * @params:@param joke
     */
    private void setOrderForJoke(String timevalue,JokeBean joke) {
		if (!timevalue.equals(indexTime)) {
			indexTime = timevalue;
			paihangindex = 1;
		} else {
			paihangindex++;
		}
		joke.paihang = paihangindex;
	}
	...
	private void saveDataToLocal(String strJson) {
		switch (REQUEST_FLAG) {
		case CHOOSE_DAY:
			pUitl.storeJokeRecordDay(strJson);
			break;
		case CHOOSE_WEEK:
			pUitl.storeJokeRecordWeek(strJson);
			break;
		case CHOOSE_MONTH:
			pUitl.storeJokeRecordMonth(strJson);
			break;
		default:
			break;
		}
		...
		if(info.getState() == HttpTaskState.STATE_OK){
			...
			case 870019:
					if(bNetWork.getReturnCode() == ReturnCode.RETURNCODE_OK){
						...
						for (JokeBean j : newJokes) {
							setOrderForJoke(j.opartime, j);
						}
//						List<JokeBean> newJokes = new ArrayList<JokeBean>();
//						for (JokeBean j : jokes) {
//							newJokes.add(getNewJoke(j.opartime, j));
//						}
						switch (REQUEST_FLAG) {
							case CHOOSE_DAY:
								m_start_day+=PAGE_SIZE_ADD;
								setDataAdapter(mXLDay, adapterDay, listDay, newJokes,isRefreshAction);
								break;
							case CHOOSE_WEEK:
								m_start_week+=PAGE_SIZE_ADD;
								setDataAdapter(mXLWeek, adapterWeek, listWeek, newJokes,isRefreshAction);
								break;
							case CHOOSE_MONTH:
								m_start_month+=PAGE_SIZE_ADD;
								setDataAdapter(mXLMonth, adapterMonth, listMonth, newJokes,isRefreshAction);
								break;
							default:
								break;
							}
		...
		if (v.getId() == R.id.iv_joke_verify) { // 审核不需要登录
			...
			if (!BGApp.isUserLogin) {
				...
				case R.id.av_zan: // 赞
					...
					jBean = (JokeBean) v.getTag();
					mActionJoke = jBean;
					XuannengRequest.getInstance().requestXuanZan(this,
							mActivity, jBean.jokeid);
					break;
...
//					jBean = (JokeBean) v.getTag();
//					mActionJoke = jBean;
//					// share.setShareContent(jBean.content, jBean.imgs.size() >
//					// 0 ? jBean.imgs.get(0).img:null);
//					jBean.doShare(share);
//					actionAdapter();
//					XuannengRequest.getInstance().requestXuanShare(this, this,
//							jBean.jokeid);
//					break;
